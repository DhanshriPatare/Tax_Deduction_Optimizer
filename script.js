...
document.getElementById("tax-form").addEventListener("submit", function (e) {
  ...
  const totalDeduction = Array.from(deductions.values()).reduce((a, b) => a + b, 0);
  ...
  else if (taxableIncome <= slab3) taxOld = 0.05 * (slab2 - slab1) + 0.2 * (taxableIncome - slab2);
  else taxOld = 0.05 * (slab2 - slab1) + 0.2 * (slab3 - slab2) + 0.3 * (taxableIncome - slab3);
...
function showSmartSuggestions(deductionsMap, age, homeSelf, homeLetOut) {
  ...
  if ((maxLimits["80C"] - deductionsMap.get("80C")) > 0)
    pq.add(new Deduction("Invest in PPF/ELSS/LIC", maxLimits["80C"] - deductionsMap.get("80C"), "80C"));

  if ((maxLimits["80D"] - deductionsMap.get("80D")) > 0)
    pq.add(new Deduction("Buy/Renew Health Insurance", maxLimits["80D"] - deductionsMap.get("80D"), "80D"));

  if ((maxLimits["80CCD"] - deductionsMap.get("80CCD")) > 0)
    pq.add(new Deduction("Contribute to NPS (Self)", maxLimits["80CCD"] - deductionsMap.get("80CCD"), "80CCD(1B)"));

  if ((maxLimits["80EEA"] - deductionsMap.get("80EEA")) > 0)
    pq.add(new Deduction("Claim more Home Loan interest", maxLimits["80EEA"] - deductionsMap.get("80EEA"), "80EEA"));

  if ((maxLimits["80TTA"] - deductionsMap.get("80TTA")) > 0)
    pq.add(new Deduction("Claim interest on savings account", maxLimits["80TTA"] - deductionsMap.get("80TTA"), "80TTA"));
  ...
  while (!pq.isEmpty()) {
    const item = pq.poll();
    suggestionText += `${index++}. ${item.name} (Section ${item.section}): ₹${item.remainingAmount.toFixed(2)}\n`;
  }
